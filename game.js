...
function update() {
    ...
    gameState.officers.forEach(officer => {
        if (officer.inTruck) {
            // Update position based on truck movement
            if (officer.truckOffset) {
            ...
            // Normal officer behavior when deployed
...
function deployOfficers() {
    ...
    gameState.officers.forEach((officer, index) => {
        ...
        // Make sure division-specific properties are set
        if (officer.division === OFFICER_DIVISIONS.CLOSE_COMBAT) {
            officer.shieldActive = true;
        }
    });
